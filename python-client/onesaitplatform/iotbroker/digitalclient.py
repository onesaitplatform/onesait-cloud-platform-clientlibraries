...
class DigitalClient(Client):
    ...
    def raw_update(self, ontology, query=None, query_type="NATIVE", data=None, where=None):
        """
        Make a insert to iot-broker service of the platform

        @param ontology     ontology name
        @param query        query expression
        @param query_type   query type ['NATIVE', 'SQL']
        ...
        if query is None:
            ...
            if where is None:
                where = {}
            log.info("Making update to ontology:{}, elements:{}".format(ontology, len(data)))
        ...
        response = self.call(RestMethods.PUT.value, url, headers=headers, body=body)
    ...
    def update(self, ontology, query=None, query_type="NATIVE", data=None, where=None):
        """
        Make a insert to iot-broker service of the platform
        ...
        _res = None
        try:
            response = self.raw_update(ontology, query, query_type, data, where)
